...
@NamedQueries({
    ...
    @NamedQuery(
            name = "getSearchEmployees",
            query = "SELECT e FROM Employee AS e WHERE e.name LIKE :search ORDER BY e.id DESC"
            ),
    @NamedQuery(
            name = "getSearchEmployeesCount",
            query = "SELECT COUNT(e) FROM Employee AS e WHERE e.name LIKE :search"
            )
